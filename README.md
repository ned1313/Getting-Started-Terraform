...
In the course, you are given a Terraform configuration for a basic web application from someone else on the team. We are going to make a copy of this configuration and modify over the length of the course. The `base_web_app` directory has the basic web app configuration, which we will copy over to the `globo_web_app` and start making alterations.
...
At the beginning of module three, we will hardcode our AWS keys into the configuration. **You would not do this in any kind of scenario!** I am trying to illustrate the point in the course. Please do not commit your AWS keys to source control. Some of us (me) have done this in the past and felt very silly. We will move these keys into environment variables in module four, and they shall never again be hardcoded **anywhere**.
...
When you complete an exercise in the course, you can easily tear down the deployed infrastructure using `terraform destory`. Just run that command and approve the destruction to remove all resources from AWS.
